...
//
mat Geometry::ComputeModelMatrix()
{
	/*
	顶点缩放，表示为
	——							 ——				——
	...
	*/
	...
	ScaleMatrix[0][0] = gMeshScale.x;
	ScaleMatrix[1][1] = gMeshScale.y;
	ScaleMatrix[2][2] = gMeshScale.z;

	// 2.计算位移矩阵
	mat MoveMatrix = mat::identity(4);
	MoveMatrix[0][3] = gMeshMove.x;
	MoveMatrix[1][3] = gMeshMove.y;
	MoveMatrix[2][3] = gMeshMove.z;

	// 3.计算旋转矩阵
	mat RotateMatrix = mat::identity(4);

	float cosX = std::cos(gMeshRotate.x * RtoPi);
	float sinX = std::sin(gMeshRotate.x * RtoPi);

	float cosY = std::cos(gMeshRotate.y *RtoPi);
	float sinY = std::sin(gMeshRotate.y * RtoPi);

	float cosZ = std::cos(gMeshRotate.z * RtoPi);
	float sinZ = std::sin(gMeshRotate.z * RtoPi);
...
//	对应OpenGL的LookAt函数
//
mat Geometry::ComputeViewMatrix()
{
	// 1.施密特正交化
	vec3f n = (gEye - gAt).normalize();  //z
	vec3f u = (gUp ^ n).normalize();	   //x
...
//	  nearZ		-  视锥体最近Z平面与原点距离
//
mat Geometry::ComputeProjectMatrix()
{
	mat projMat = mat::identity(4);
	// cot(fovy* pi / 360)
	float cotFovyDiv2 = 1.f / tan(gFovy*RtoPi / 2.f);

	//使用左手坐标系
	projMat[0][0] = cotFovyDiv2 / gAspect;
	projMat[1][1] = cotFovyDiv2;
	projMat[2][2] = gFarZ / (gFarZ - gNearZ);
	projMat[2][3] = gNearZ*gFarZ / (gFarZ - gNearZ);
	projMat[3][2] = -1;
	projMat[3][3] = 0;
...
// 计算MVP矩阵
mat Geometry::ComputeMVP()
{
	mat M = ComputeModelMatrix();
	mat V = ComputeViewMatrix();
	mat P = ComputeProjectMatrix();
